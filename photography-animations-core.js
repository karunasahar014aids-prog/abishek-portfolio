...
(function(){
  ...
  function freshImageUrl(url){const raw=String(url||'');if(!raw)return raw;return raw+(raw.includes('?')?'&':'?')+'v='+Date.now();}
  function createLiveCard(item){
    const card=document.createElement('div');card.className='g-card reveal live-admin-photo';card.dataset.category=categoryKey(item.category);card.dataset.liveId=String(item.id||'');
    ...
    card.addEventListener('click',e=>{if(e.target.closest('.g-dots,.g-menu,.g-fav-badge'))return;e.preventDefault();e.stopPropagation();openLiveModal(item);},true);return card;
  }
  function openLiveModal(item){
    const back=document.getElementById('modal-backdrop');const img=document.getElementById('modal-img');if(!back||!img)return;
    img.src=freshImageUrl(item.image_url);img.alt=item.title||'Photograph';
    ...
    const specs=document.getElementById('modal-specs');if(specs)specs.innerHTML=[['Location',item.location||'Studio'],['Date',item.date||'']].map(([l,v])=>`<div class="spec-item"><div class="spec-label">${esc(l)}</div><div class="spec-val">${esc(v)}</div></div>`).join('');
    const tags=document.getElementById('modal-tags');if(tags)tags.innerHTML='';const related=document.getElementById('modal-related-row');if(related)related.innerHTML='';
    if(!back.classList.contains('open')&&typeof lockPageScroll==='function')lockPageScroll();back.classList.add('open');
  }
  ...
  function initPhotoZoom(){
    const wrap=document.querySelector('.modal-img');const img=document.getElementById('modal-img');const modal=document.getElementById('modal');if(!wrap||!img||!modal)return;if(wrap.dataset.zoomReady==='1')return;wrap.dataset.zoomReady='1';wrap.classList.add('zoom-scroll');let level=1;
    const controls=document.createElement('div');controls.className='photo-zoom-controls';controls.setAttribute('aria-label','Photo zoom');
    [1,2,3].forEach(value=>{const btn=document.createElement('button');btn.type='button';btn.textContent=value+'x';btn.dataset.zoom=String(value);btn.setAttribute('aria-label','Zoom '+value+'x');btn.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();setZoom(value);});controls.appendChild(btn);});wrap.appendChild(controls);
    function setZoom(value){level=Math.max(1,Math.min(3,value));img.style.transform='scale('+level+')';img.style.transformOrigin='center center';img.style.cursor=level>1?'zoom-out':'zoom-in';controls.querySelectorAll('button').forEach(b=>b.classList.toggle('active',Number(b.dataset.zoom)===level));}
    img.addEventListener('click',()=>setZoom(level===1?2:1));wrap.addEventListener('wheel',e=>{if(!document.getElementById('modal-backdrop')?.classList.contains('open'))return;e.preventDefault();setZoom(level+(e.deltaY<0?1:-1));},{passive:false});setZoom(1);const reset=()=>setZoom(1);const observer=new MutationObserver(()=>{if(img.getAttribute('src'))reset();});observer.observe(img,{attributes:true,attributeFilter:['src']});modal.addEventListener('click',e=>{if(e.target===modal)reset();});
  }
  function observeModal(){const back=document.getElementById('modal-backdrop');if(!back||back.dataset.zoomObserver==='1')return;back.dataset.zoomObserver='1';const tryInit=()=>setTimeout(initPhotoZoom,30);new MutationObserver(tryInit).observe(back,{attributes:true,childList:true,subtree:true,attributeFilter:['class','style','src']});document.addEventListener('click',e=>{if(e.target.closest('.g-card,.modal-next,.modal-prev,.modal-related-row'))tryInit();},true);tryInit();}
