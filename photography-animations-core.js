...
(function(){
  ...
  function categoryFromImage(card){
    ...
    if(src.includes('/gallery/street-'))return'street';
  ...
  function updateButtons(){
    ...
    bar.querySelectorAll('.filter-btn').forEach(b=>{
      const key=categoryKey(b.dataset.cat||b.textContent);
      b.dataset.cat=key;
      b.classList.toggle('active',key===activeFilter);
      b.setAttribute('aria-selected',key===activeFilter?'true':'false');
    });
  }
  ...
  function ensureFilterButtons(){
    ...
    if(!bar.dataset.finalGalleryController){
      bar.dataset.finalGalleryController='1';
      bar.innerHTML='';
  ...
  function staticCards(){
    const g=gallery();
    if(!g)return[];
    return Array.from(g.querySelectorAll('.g-card:not(.live-admin-photo)'));
  }
  ...
  function createLiveCard(item){
    ...
    card.innerHTML=`<div class="g-card-img"><img src="${esc(item.image_url)}" alt="${esc(item.title||'Photograph')}" loading="lazy"><div class="g-overlay"><div class="g-cat">${esc(item.category||'Photography')}</div><div class="g-title">${esc(item.title||'New Photograph')}</div><div class="g-loc">${esc(item.location||'Studio')} · ${esc(item.date||'')}</div></div></div>`;
    return card;
  }
  ...
  function observeGallery(){
    const g=gallery();
    if(!g||g.dataset.finalObserver)return;
    g.dataset.finalObserver='1';
