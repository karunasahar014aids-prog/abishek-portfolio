...
(function(){
  ...
  function ensureFilterButtons(){
    ...
    if(!bar.dataset.finalGalleryController){
      ...
      bar.addEventListener('click',e=>{
        ...
        /* renderGallery() clears the gallery DOM, so restore the Supabase photographs immediately after rendering the selected static category. */
  ...
  function style(){if(document.getElementById('final-gallery-style'))return;const s=document.createElement('style');s.id='final-gallery-style';s.textContent='#filter-bar{display:flex;gap:10px;flex-wrap:wrap;justify-content:center}#filter-bar .filter-btn{cursor:pointer}#filter-bar .filter-btn.active{font-weight:700}#gallery .g-card{visibility:visible}#gallery .g-card img{display:block;width:100%}#modal-img{transform-origin:center center;transition:transform .22s ease;cursor:zoom-in}.photo-zoom-controls{position:absolute;left:50%;bottom:12px;transform:translateX(-50%);display:flex;gap:6px;z-index:20;padding:6px;border:1px solid rgba(255,255,255,.18);border-radius:999px;background:rgba(0,0,0,.72);backdrop-filter:blur(10px)}.photo-zoom-controls button{border:0;border-radius:999px;padding:6px 11px;background:transparent;color:#fff;font:600 12px/1 system-ui,sans-serif;cursor:pointer}.photo-zoom-controls button.active{background:#FFD400;color:#080808}.photo-zoom-controls button:hover{background:rgba(255,255,255,.15)}.photo-zoom-controls button.active:hover{background:#FFD400}.modal-img.zoom-scroll{overflow:auto!important;overscroll-behavior:contain}';document.head.appendChild(s);}
