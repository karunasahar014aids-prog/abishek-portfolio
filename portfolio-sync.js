...
(function(){
  ...
  function addLiveGalleryStyles(){
    ...
    s.textContent=`#gallery .live-admin-photo{position:relative;overflow:hidden;border-radius:18px;background:#eef2f7;margin:0 0 22px;break-inside:avoid;cursor:pointer;box-shadow:0 10px 30px rgba(17,24,39,.10);transform:translateY(18px);opacity:0;transition:transform .7s ease,opacity .7s ease,box-shadow .3s ease}#gallery .live-admin-photo.live-visible{transform:translateY(0);opacity:1}#gallery .live-admin-photo:hover{box-shadow:0 18px 45px rgba(17,24,39,.18)}#gallery .live-admin-photo img{display:block;width:100%;height:auto;min-height:220px;object-fit:cover;transition:transform .7s ease}#gallery .live-admin-photo:hover img{transform:scale(1.045)}#gallery .live-admin-photo .live-photo-info{position:absolute;left:0;right:0;bottom:0;padding:28px 18px 16px;color:#fff;background:linear-gradient(transparent,rgba(0,0,0,.78))}.live-gallery-empty{padding:18px;border:1px dashed #dbe1ea;border-radius:14px;color:#667085;font-size:13px}`;
  ...
  function addLivePhotoToGallery(p){
    ...
    if([...gallery.querySelectorAll('.live-admin-photo img')].some(img=>img.src===p.image_url))return;

    const card=document.createElement('article');
    card.className='live-admin-photo';
    card.dataset.adminPhotoId=p.id;
    card.dataset.cat=(p.category||'other').toLowerCase();
    card.innerHTML=`<img src="${esc(p.image_url)}" alt="${esc(p.title||'Photography')}" loading="lazy"><div class="live-photo-info"><div>${esc(p.category||'Photography')}</div><div>${esc(p.title||'Untitled Photograph')}</div><div>${esc(p.location||'')} ${p.date?'• '+esc(p.date):''}</div></div>`;
    card.addEventListener('click',()=>window.open(p.image_url,'_blank','noopener'));
    gallery.prepend(card);
    requestAnimationFrame(()=>requestAnimationFrame(()=>card.classList.add('live-visible')));
  }

  async function syncPublishedPhotos(){
    if(!document.getElementById('gallery') || !sb)return;
    try{
